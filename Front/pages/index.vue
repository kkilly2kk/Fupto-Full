...
try {
  // 브랜드 데이터 가져오기
  const { data: brandData } = await useAuthFetch("/brands", {
    params: {
  ...
  if (brandData.value) {
    designers.value = brandData.value; // 브랜드 응답 데이터 저장
  }

  // 제품 데이터 가져오기
  const { data: productData } = await useAuthFetch("/products", {
    params: {
      cursor: null,
      limit: 100,
    },
  ...
  if (productData.value) {
    products.value = productData.value.products; // 제품 응답 데이터 저장
  }
} catch (error) {
  ...
  <div id="app" class="main-container">
        ...
        <div class="user-product-list">
          <ul class="product-grids">
            <template v-if="products.length > 0">
              <li v-for="(product, index) in products" :key="product.id" class="product-c-frame">
    ...
    <div class="watching-section">
      ...
      <div class="text-container">
        ...
        <p class="description">
          Refresh your wardrobe with exclusive designs from top-tier designers. Let go of those deep sale purchases that no longer
          serve you, and discover curated luxury fashion
        </p>
        <a href="https://www.pentagram.com/work/mytheresa" target="_blank" class="cta-button"> WATCH </a>
      </div>
    </div>
  </div>
</template>
